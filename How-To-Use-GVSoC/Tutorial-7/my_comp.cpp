...
class MyComp : public vp::Component
{
    ...
    private:
        ...
        // Create vp::ClockEvent object to handle clock events
        vp::ClockEvent event;

        // Create vp::ioReq object to handle pending asyncronous I/O requests
        vp::IoReq *pending_req;

        // Declare event-handling function
...
vp::IoReqStatus MyComp::handle_req(vp::Block *__this, vp::IoReq *req)
{
    ...
    */
    if (!req->get_is_write())
    {
        ...
        {
            //Add latency (1000 cycles) for syncronous response
        ...
        else if (req->get_addr() == 4 && req->get_size() == 4)
        {
            // Save the pending request
            _this->pending_req = req;

            // Add wait time (not lantency) of 2000 cycles. After this, it will call the function handle_event()
            _this->event.enqueue(2000);

            // Return status of pending I/O request
...
// Handle_event() function for event object for async response
void MyComp::handle_event(vp::Block *__this, vp::ClockEvent *event)
{
    MyComp *_this = (MyComp *)__this;

    // Put data into response buffer of request
    *(uint32_t *)_this->pending_req->get_data() = _this->value;

    // Send response back. This way is for asyncronous repsonse, which differs from syncronous response method
