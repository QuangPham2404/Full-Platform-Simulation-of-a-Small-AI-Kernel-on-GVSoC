...
class MyComp : public vp::Component
{
    ...
    private:
        ...
        // Add clk ctrl interface to control clock domain 1's clock freq
        vp::ClockMaster clk_ctrl_itf;
        // Add event object to help with scheduling the changing of clock freq (tutorial 7)
        vp::ClockEvent event;
        // Add a variable to store the next freq to change to
        int frequency;
        // Declare function handle_event() to change the clock frequency
        static void handle_event(vp::Block *__this, vp::ClockEvent *event);
        // Declare reset fuction
...
MyComp::MyComp(vp::ComponentConf &config)
    : vp::Component(config), vcd_value(*this, "status", 32) , event(this, &MyComp::handle_event) // The vcd_value signal belongs to my_comp, it has the name status, and it is 32 bits wide
{
    ...
    // Register the clock control output port under the name "clk_ctrl" defined in the Python script
...
// Function: to start the "oscillating" process of frequency by calling the event if active is False (it is at the start)
// It starts by setting frequency to 10 MHz and enqueue the event (adding 100 cycle delay) to call the handle_event() functino
// After 100 cycles, handle_event() will be executed for the first time, and then the oscillation of freq starts
void MyComp::reset(bool active)
{
    if (!active)
    {
        this->frequency = 10000000; // 10 MHz
        this->event.enqueue(100);
    }
}

// Even handler function to implement frequency change
// Behaviour: Alternate between 1 GHz + wait 1000 cycles and 10 MHz + 100 cycles
void MyComp::handle_event(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    // Prepare the next frequency and next event
    if (_this->frequency == 10000000)
    {
        _this->frequency = 1000000000; // next = 1 GHz
        _this->event.enqueue(1000);
    }
    else
    {
        _this->frequency = 10000000;   // next = 10 MHz
